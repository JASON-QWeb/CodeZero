...
describe("web settings utilities", () => {
  ...
  it("serves sanitized settings demo data without network calls", async () => {
    ...
    expect(config.rootDir).toBe("/demo/codezero");
    ...
    await expect(saveProviderApiKey({ providerId: "default", apiKey: "demo-key" })).resolves.toMatchObject({ saved: true });
    await expect(
      updateRepositoryRuntimeSettings({
        repositoryId: "demo-labs/nova-commerce",
        triggerMode: "manual",
        mention: "@agent-prd",
        maxConcurrentIssues: 2,
        projectSkillPath: ".agent",
