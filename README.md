...
- WebUI 配置：Settings Console 可编辑并校验 `agents.yaml`、`repositories.yaml`、`tools.yaml`、`policies.yaml` 和 `sandbox.yaml`，提供仓库 trigger/并发/权限快捷表单，覆盖模型、仓库、触发方式、仓库级工具权限、Policy 和沙箱，并可点击 Test 验证模型 API key/base URL/model 是否可用。
