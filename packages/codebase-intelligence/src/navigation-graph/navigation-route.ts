...
const likelyEntrypointPatterns = [
  "/app/",
  "/pages/",
  "/routes/",
  "/api/",
...
export function buildNavigationRoute(
  input: BuildNavigationRouteInput,
): NavigationRoute {
  const terms = extractTerms(
    [input.issue.title, input.issue.body, input.issue.labels.join(" ")].join(
      "\n",
    ),
  );
  ...
  for (const file of input.files) {
    ...
    if (
      likelyEntrypointPatterns.some(
        (pattern) =>
          lowerPath.includes(pattern) || lowerPath.startsWith(pattern.slice(1)),
      )
    ) {
      evidence.score += 2;
  ...
  for (const symbol of input.symbols) {
    ...
    if (matchingTerm) {
      ...
      evidence.reasons.push(
        `Symbol ${symbol.name} matched issue term "${matchingTerm}"`,
      );
    }
  }
  ...
  for (const edge of input.graph.edges) {
    if (edge.kind === "handles_route" && edge.to.startsWith("file:")) {
      ...
      if (matchingTerm) {
        ...
        evidence.score += 7;
        evidence.reasons.push(
          `Route ${edge.from} matched issue term "${matchingTerm}"`,
        );
      }
    }

    if (
      edge.kind === "mentions_business_concept" &&
      ...
      if (terms.includes(concept)) {
        const evidence = ensureEvidence(
          fileScores,
          edge.to.slice("file:".length),
        );
        evidence.score += 5;
        evidence.reasons.push(
          `Business concept "${concept}" matched issue text`,
        );
      }
    }
  }
  ...
  const doNotModify = input.files
    ...
    .filter(
      (filePath) =>
        !mustRead.includes(filePath) &&
        riskyPathTerms.some((term) => filePath.toLowerCase().includes(term)),
    )
    .slice(0, 12);
...
function propagateGraphEvidence(
  graph: RepoNavigationGraph,
  fileScores: Map<string, NavigationRouteEvidence>,
): void {
  ...
  for (const edge of graph.edges) {
    ...
    if (
      activeFiles.has(from) &&
      ["imports", "tests", "changed_with"].includes(edge.kind)
    ) {
...
function findRouteTests(
  ...
  return graph.edges
    ...
    .map((edge) => edge.to.slice("file:".length));
}
...
function findEntrypoints(
  ...
  if (routes.length > 0) {
    return routes.filter(isUniqueValue);
  }
...
function ensureEvidence(
  scores: Map<string, NavigationRouteEvidence>,
  filePath: string,
