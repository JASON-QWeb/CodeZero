...
export async function buildRepoNavigationGraph(
  input: BuildRepoNavigationGraphInput,
): Promise<RepoNavigationGraph> {
  ...
  for (const symbol of input.symbols) {
    ...
    edges.push({
      ...
      confidence: 0.9,
      evidence: [
        `${symbol.kind} ${symbol.name} defined at line ${symbol.line}`,
      ],
    });
  }
  ...
  edges.push(
    ...(await buildImportEdges(input.repoDir, input.files, filePaths)),
  );
  edges.push(
    ...buildBusinessConceptEdges(input.businessRules ?? [], input.files, nodes),
  );

  if (input.includeGitHistory ?? true) {
    edges.push(
      ...(await buildChangedWithEdges(
        input.repoDir,
        filePaths,
        input.maxGitCommits ?? 80,
      )),
    );
  }
  ...
  return {
    ...
    stats: {
      ...
      routes: Array.from(nodes.values()).filter((node) => node.kind === "route")
        .length,
      imports: dedupedEdges.filter((edge) => edge.kind === "imports").length,
      tests: dedupedEdges.filter((edge) => edge.kind === "tests").length,
      changedWith: dedupedEdges.filter((edge) => edge.kind === "changed_with")
        .length,
    },
...
function buildTestEdges(files: FileIndexEntry[]): RepoGraphEdge[] {
  ...
  for (const test of files.filter((file) => file.isTest)) {
    ...
    const testBase = basenameWithoutExtensions(testPath).replace(
      /\.(test|spec)$/i,
      "",
    );
    const source =
      nonTestFiles.find(
        (filePath) => basenameWithoutExtensions(filePath) === testBase,
      ) ??
      nonTestFiles.find((filePath) =>
        testPath.includes(path.posix.dirname(filePath)),
      );
...
async function buildImportEdges(
  ...
  for (const file of codeFiles) {
    ...
    for (const specifier of collectImportSpecifiers(file.path, content)) {
...
function collectImportSpecifiers(filePath: string, content: string): string[] {
  const source = ts.createSourceFile(
    filePath,
    content,
    ts.ScriptTarget.Latest,
    true,
    scriptKind(path.extname(filePath)),
  );
...
function buildBusinessConceptEdges(
  businessRules: string[],
  files: FileIndexEntry[],
...
async function buildChangedWithEdges(
  repoDir: string,
  filePaths: Set<string>,
  maxGitCommits: number,
): Promise<RepoGraphEdge[]> {
...
function routeFromPath(filePath: string): string | undefined {
  ...
  if (
    /^app\/.*\/page\.[jt]sx?$/.test(normalized) ||
  ...
  if (
    /^pages\/api\/.*\.[jt]sx?$/.test(normalized) ||
    /(^|\/)(routes|api)\//.test(normalized)
  ) {
